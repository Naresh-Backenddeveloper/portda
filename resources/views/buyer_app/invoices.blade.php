...
<title>Invoices Â· PORTDA Buyer</title>
...
<div class="app-shell">
  <aside class="sidebar">
    ...
    <nav class="sidebar-nav">
      ...
      <a class="nav-item active" href="/buyer/invoices"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/></svg><span>Invoices</span></a>
  ...
  <header class="topbar">
    <div class="search"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg><input type="search" placeholder="Search invoicesâ€¦" /></div>
  ...
  <main class="main">
    <div class="page-header">
      <div class="page-title"><h1>Invoices</h1><p>GST-compliant tax invoices from every vendor. Auto-filed by your GSTIN.</p></div>
      <div class="page-actions"><button class="btn btn-outline">Export GSTR-2</button><button class="btn btn-primary">Download all</button></div>
    </div>

    <div class="kpi-grid">
      <div class="kpi"><div class="kpi-label">This month</div><div class="kpi-value">12 invoices</div><div class="kpi-delta">â‚¹18.4 L</div></div>
      <div class="kpi"><div class="kpi-label">GST input credit</div><div class="kpi-value">â‚¹2.81 L</div><div class="kpi-delta up">claimable</div></div>
      <div class="kpi"><div class="kpi-label">Pending</div><div class="kpi-value">4</div><div class="kpi-delta down">â‚¹3.4 L total</div></div>
      <div class="kpi"><div class="kpi-label">FY 26â€“27 spend</div><div class="kpi-value">â‚¹3.8 Cr</div></div>
    </div>

    <div class="tab-strip">
      <a href="#" class="active">All (172)</a><a href="#">This month (12)</a><a href="#">Pending (4)</a><a href="#">Paid (168)</a><a href="#">Disputed (0)</a>
    </div>

    <div class="table-wrap">
      <table class="t">
        <thead><tr><th>Invoice #</th><th>Vendor</th><th>Order</th><th>Date</th><th>Subtotal</th><th>GST</th><th>Total</th><th>Status</th></tr></thead>
        <tbody>
          @forelse ($items as $inv)
            <tr>
              <td><strong>{{ $inv->number }}</strong></td>
              <td><div class="t-buyer"><div class="avatar b1">{{ strtoupper(substr($inv->vendor->name ?? 'V',0,2)) }}</div><div><div class="name">{{ $inv->vendor->name ?? '—' }}</div></div></div></td>
              <td>#{{ $inv->order->reference ?? '—' }}</td>
              <td>{{ $inv->issued_at ? $inv->issued_at->format('d M') : '—' }}</td>
              <td>₹{{ number_format($inv->subtotal) }}</td>
              <td>₹{{ number_format($inv->tax) }}</td>
              <td class="t-amount">₹{{ number_format($inv->total) }}</td>
              <td><span class="chip @if($inv->status==='paid') chip-success @elseif($inv->status==='overdue') chip-warning @else chip-primary @endif">{{ ucfirst($inv->status) }}</span></td>
            </tr>
          @empty
            <tr><td colspan="8" class="muted" style="text-align:center;padding:32px;">No invoices yet.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
    ...
    <div class="grid-2 mt-20">
      <div class="card">
        <div class="card-head"><h3>GST input credit summary</h3><span class="chip chip-success">Auto-claim ON</span></div>
        <div class="row-between" style="padding:8px 0;border-bottom:1px solid var(--border-2);"><span class="muted">FY 26â€“27 (to date)</span><strong>â‚¹2,81,420</strong></div>
        <div class="row-between" style="padding:8px 0;border-bottom:1px solid var(--border-2);"><span class="muted">This month (MTD)</span><strong>â‚¹38,420</strong></div>
        <div class="row-between" style="padding:8px 0;border-bottom:1px solid var(--border-2);"><span class="muted">Pending UTR (cannot claim yet)</span><strong style="color:var(--warning);">â‚¹1,06,520</strong></div>
        <div class="row-between" style="padding:8px 0;"><span class="muted">Last filing (GSTR-2A match)</span><strong style="color:var(--success);">100%</strong></div>
      </div>
      <div class="card">
        <div class="card-head"><h3>Quick actions</h3></div>
        <button class="btn btn-outline btn-block mb-12">Generate GSTR-2 export (May)</button>
        <button class="btn btn-outline btn-block mb-12">Reconcile with Tally</button>
        <button class="btn btn-outline btn-block">Email all invoices to finance@oceanlink.in</button>
      </div>
    </div>
  </main>
</div>
