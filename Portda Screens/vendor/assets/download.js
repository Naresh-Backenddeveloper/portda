// PORTDA Vendor — per-screen download. Mirrors the user-app version.
(function () {
  function init() {
    if (typeof html2canvas === 'undefined') return setTimeout(init, 200);
    document.querySelectorAll('.screen-wrap').forEach(function (wrap) {
      ...
      btn.addEventListener('click', function (e) {
        ...
        btn.dataset.busy = '1'; btn.classList.add('is-busy');
        html2canvas(phone, { scale: 3, backgroundColor: null, useCORS: true, logging: false }).then(function (canvas) {
          var raw = label.textContent.replace('PNG', '').trim();
          var safe = raw.replace(/[^\w\s.-]/g, '').replace(/\s+/g, '_').replace(/_+$/, '');
          var filename = 'PORTDA_Vendor_' + (safe || 'screen') + '.png';
          canvas.toBlob(function (blob) {
            ...
            btn.classList.remove('is-busy'); delete btn.dataset.busy;
          });
        }).catch(function () {
          btn.classList.remove('is-busy'); delete btn.dataset.busy;
